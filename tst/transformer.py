...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.layers_encoding = nn.ModuleList([Encoder(d_model,
                                                      ...
                                                      conformer = conformer,
                                                      conv_dimen = conv_dimen) for _ in range(N)])
        ...
        self._linear = nn.Linear(d_model, 4)
        #self._output_layer = nn.Linear(768, 4)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        output_en = self._linear(encoding)
        #flatten
        #output_en = output_en.view(output_en.shape[0],-1)
        
        #output_en = self._output_layer(output_en)
        #statistical pooling
        if self.embedding_option == 'mean':
            output_en = torch.mean(output_en,dim = 1)
        elif self.embedding_option == 'last': 
